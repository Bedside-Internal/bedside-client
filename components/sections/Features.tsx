...
import type { FormatCardDTO, FormatCardAccent } from "@/types/marketing";

const delays = ["", "d1", "d2"] as const;

const accentHex: Record<FormatCardAccent, string> = {
  mint: "#3BBA9C",
  coral: "#E85D4A",
  amber: "#F9A03F",
  violet: "#7C6AF7",
...
export default function Features({ formatCards }: { formatCards: FormatCardDTO[] }) {
  return (
      ...
      <div className="mx-auto max-w-[1160px]">
        ...
        <div className="grid grid-cols-1 gap-[18px] sm:grid-cols-2 lg:grid-cols-3">
          {formatCards.map((card, i) => (
            <RevealOnScroll key={card.id} delay={delays[i % 3]} className="group relative">
              <div
                aria-hidden
              ...
              <div
                ...
                style={{ borderTopColor: accentHex[card.accent] }}
              >
                <div
                  className="inline-block h-2.5 w-2.5 rounded-full border-2 border-ink"
                  style={{ background: accentHex[card.accent] }}
                />
